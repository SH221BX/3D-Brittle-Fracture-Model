function segs3 = finding_riverlines_tri(triV, phi1, Phi, phi2, stepSpacing, nSteps, ridgeWidth, ridgeHeight, faceColor)

if nargin < 5 || isempty(stepSpacing), stepSpacing = 0.35; end
if nargin < 6 || isempty(nSteps),      nSteps      = 60;   end
if nargin < 7 || isempty(ridgeWidth),  ridgeWidth  = 0.20; end
if nargin < 8 || isempty(ridgeHeight), ridgeHeight = 0.25; end
if nargin < 9 || isempty(faceColor),   faceColor   = [0.93 0.69 0.13]; end
...
n2 = rotated_planes(2,:).'; n2 = n2/norm(n2);

P0 = triV;
if size(P0,1) < 3, return; end

nb = cross(P0(2,:)-P0(1,:), P0(3,:)-P0(1,:));
...
%hVals = linspace(0, 0.5*ridgeHeight, nBands);
...
for k = 1:nBands
    ...
    if k > 1
        ...
        if ~isempty(Q)
            ...
            % patch('Vertices',[A0; B0; B1; A1],'Faces',[1 2 3 4], ...
            %     'FaceColor',faceColor,'FaceAlpha',0.55, ...
            %     'EdgeColor','none'); hold on
            ...
            fcSide = shadeColor(darkenColor(baseCol, 0.82), nr, lightDir, ambient, diffuse);

            patch('Vertices',riserVerts,'Faces',[1 2 3 4], ...
                'FaceColor',fcSide,'FaceAlpha',0.8, ...
                'EdgeColor','none'); hold on

            plot3([A1(1) B1(1)], [A1(2) B1(2)], [A1(3) B1(3)], ...
    'Color', darkenColor(baseCol,0.45), 'LineWidth', 0.9);
